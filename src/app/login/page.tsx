  ...
  const resolveAuthState = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(serverError ?? "bootstrap_check_failed");
      }

      if (typeof body.authenticated !== "boolean" || typeof body.requiresSetup !== "boolean") {
        throw new Error("bootstrap_check_invalid_payload");
      }

      // Setup state has higher priority than auth redirect.
      if (body.authenticated && !body.requiresSetup) {
        router.replace("/");
        return;
      }

      setMode(body.requiresSetup ? "setup" : "login");
      ...
      setMode(null);
      if (error instanceof Error && error.message && !error.message.startsWith("bootstrap_check_")) {
        setRequestError(error.message);
      } else {
        setRequestError("Vault is still starting. Wait 10-20 seconds, then retry.");
      }
    } finally {
      if (!signal?.aborted) {
        setLoading(false);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setRequestError(
          isSetupMode
            ? (json?.error ?? "Setup failed. Please try again.")
      ...
      setRequestError(
        isSetupMode
          ? "Setup failed because the app could not be reached. Please try again."
      ...
      <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-vault-canvas px-6 py-12">
        ...
        <div className="relative mx-auto w-full max-w-md rounded-2xl border border-vault-border bg-vault-surface/95 p-6 shadow-2xl backdrop-blur">
          ...
          <p className="text-sm text-vault-text-muted">
            {requestError ?? "Unable to verify vault state. Retry to continue."}
          </p>
          <button
            type="button"
